...
INPUT_JOB_POSTING_FILE = os.path.join(os.path.dirname(__file__), "sample_job_post_consulting.md")
INPUT_USER_PROFILE_FILE = os.path.join(os.path.dirname(__file__), "user_profile.json")
OUTPUT_DIRECTORY = "/home/apatel/fn_projects/roboresume/tests/test_data/v4/output"
MODEL_NAME = "gpt-4o"
...
def step_2_build_work_experience(profile: dict, analysis: IdealCandidateProfile, job_desc: str) -> List[dict]:
    """Selects and rewrites work experience from the user profile."""
    print("  Running Step 2: Building Work Experience Section...")
    
    # We provide the AI with the necessary context to make its selections
    user_prompt = (
    ...
    response = client.chat.completions.create(
        model=MODEL_NAME,
        response_model=GeneratedResume, # We ask for the whole resume structure to get work_experience
        messages=[
...
def step_3_build_skills(profile: dict, analysis: IdealCandidateProfile) -> List[dict]:
    ...
    response = client.chat.completions.create(
        model=MODEL_NAME,
        response_model=GeneratedResume, # We ask for the whole resume structure to get skills
        messages=[
...
def step_4_build_summary(built_experience: List[dict], built_skills: List[dict], analysis: IdealCandidateProfile) -> str:
    """Writes the summary based on the already-built sections."""
    print("  Running Step 4: Writing Summary...")
    
    # We show the AI the content we've already decided to include in the resume
    built_sections = {
...
def run_pipeline():
    ...
    final_resume_content = {
        ...
        "skills": [skill.model_dump() for skill in built_skills],
        "target_role": analysis_profile.experience_summary # Use analysis for target_role
    }
    ...
    try:
        # Save the final resume content in the format the PDF generator needs
        with open(os.path.join(temp_session_dir, "tailored_resume_content.json"), "w") as f:
            json.dump(final_resume_content, f, indent=4)
            
        # Create a temporary base_resume for the generator, containing personal_info
        ...
        # Create a mock job analysis for the generator
        with open(os.path.join(temp_session_dir, "structured_job_data.json"), "w") as f:
             json.dump({"company_name": "Target Company"}, f, indent=4)
